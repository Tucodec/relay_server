...
inline uint64_t htonll2(uint64_t num)
{
    int i = 0x1;
    uint64_t retval;
    if(*(char *)&i == 0x1) {
...
inline uint64_t ntohll2(uint64_t num)
{
    int i = 0x1;
    uint64_t retval;
    if(*(char *)&i == 0x1) {
...
{
    uint64_t network_byteorder=Phata::htonll2(intnum);//转换为网络字节序
...
inline bool CharToInt64(const unsigned char* charnum, uint64_t& intnum)
{
    ...
    intnum=Phata::ntohll2(intnum);//转换为网络字节序
    return true;
}
...
typedef struct _NetPacket{//传输自定义数据包头结构
	uint32_t version;        //封包的版本号，不同版本包的定义可能不同  :0-3
	...
	uint32_t type;           //包数据的类型                            :22-25
	uint32_t datalen;        //包数据的内容长度-不包括此包结构和包头尾 :26-29
	uint32_t reserve;        //包数据保留字段-暂时不使用               :30-33
}NetPacket;
